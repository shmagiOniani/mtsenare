...
function Chat(props) {
  ...
  let hours = JSON.stringify(moment().hours())?.length === 1 ? `0${moment().hours()}`: moment().hours();
  let minutes =  moment().minutes()
  let currentDate = `${moment(today).month(today.getMonth()).format("YYYY-MM-DD") } ${hours}:${minutes}`;
  ...
  const onTextChange = (e) => {
    setState({ ...state, [e.target.name]: e.target.value , name: context.user.username})
  }

  const onFinish = (e) => {
    
    const { name, text } = state
      API.post(`/post-requests-handler`, { url: `/Chat/messages`, params: {}, data: {text, user: userObj.username,branch: branch.name, timestamp: `${currentDate}`} })
    ...
    return chat?.map(({ user, text ,branch , timestamp}, index) => (
      <div key={index} className={`chat-item ${user === userObj.username}`}>
        ...
        <div className="date-filial">
          ...
          <div className="date">{`${timestamp?.slice(0,10)} / ${timestamp?.slice(11,16)}`}</div>
        </div>
      </div>
    ))
  }
  ...
  const getMessages = (page, count) => {
    API.get(`/get-requests-handler`, { params: { url: `/Chat/messages`, params: {Page: page, Count: count} }  })
    .then(
      (res) => {
        if (res.data.name === "Error") {
          message.error(res.data);
      ...
      <div className='chat-wrapper '>
        ...
        <div className="notification-input">
          <form onSubmit={onFinish}>
            <TextArea
              onKeyPress={(board)=> board.key === "Enter" ? onFinish(board) : ""}
              onChange={(e) => onTextChange(e)}
              name="text"
              rows={3}
              value={state.text} />
            <Button className='focus-white' htmlType='submit' type={"primary"}>{trans("submit")}</Button>
          </form>
        </div>
      </div>
    </div>
