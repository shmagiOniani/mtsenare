...
function CarFault({ treeClone, tree, getCheckboxArr, handleFinish }) {
  ...
  const colorCollapses = (object) => {
    ...
    if (object?.parentId === null) {
      ...
      arr.push(object.id.toString());
      setColoredCollapseIds(arr);
      const nextObj = treeClone.find(
        (item) => item.item.id === object.parentId
      );
  ...
  const handleCheckboxRec = () => {
    if (checkBoxArr?.length > 0) {
      let result = treeClone.filter((obj) =>
        checkBoxArr?.includes(obj?.item?.id)
      );
  ...
  const handleCheckbox = (id, obj) => {
    const filter = checkBoxArr?.filter(
      (checkBoxItemId) => checkBoxItemId !== id
    );
    const filterFaultArr = faultArr?.filter(
      (checkBoxItemName) => checkBoxItemName.name !== obj.name
    );
    // checkbox logic
  ...
  const callback = (key, obj) => {
    ...
    if (!obj.parentId) {
      ...
      if (
        activeCollapseIds.includes(prevSetCollObj?.parentId) &&
        obj.parentId === prevSetCollObj?.parentId
      ) {
        if (activeCollapseIds.includes(obj.id)) {
          ...
          const instance = activeCollapseIds.filter(
            (item) => item !== prevSetCollObj.id
          );
        ...
        let instance = activeCollapseIds.filter(
          (item) => item !== prevSetCollObj.id
        );
        ...
        if (prevSetCollObj.parentId !== null) {
          const prevObjGpar = treeClone.find(
            (item) => item.item.id === prevSetCollObj.parentId
          );
          if (prevObjGpar.item.parentId === obj.parentId) {
            const filterChild = activeCollapseIds.filter(
              (item) => item !== prevSetCollObj.id
            );
            const filterChildParent = filterChild.filter(
              (item) => item !== prevSetCollObj.parentId
            );
      ...
      <div key={leaf?.item?.id}>
        ...
        {leaf?.item?.leaf ? (
          ...
          >
            <div
              ...
              onClick={() =>
                console.log(
                  "data",
                  treeClone.find((i) => i?.item.id === leaf?.item.parentId)
                )
              }
            >
              <Checkbox
          ...
          >
            <Panel
              className={` ${
                coloredCollapseIds.includes(leaf?.item?.id.toString())
                  ? "marked-collapse"
                  : ""
              }`}
  ...
  const handleVehicleApi = () => {
    API.get(`/get-requests-handler`, {
      ...
      .then((res) => {
        ...
        const rejectedArr = res.data?.rejectedDetails?.map(
          (item) => item.rejectedDetailId
        );
        const initNamesObjs = treeClone?.filter((item) =>
          rejectedArr?.includes(item?.item?.id)
        );
      ...
      <div className="car-fault">
        ...
        {faultArr.length > 0 ? (
          <div className="car-fault-list ">
            <div className="car-fault-header">{trans("defects_found")}</div>
            <ul>
